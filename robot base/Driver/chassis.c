...
#define RED_MAX_SPEED 180.0 //120
#define BLUE_MAX_SPEED 120.0
float MAX_SPEED = 120;
...
{
	extern int color;
	if (color==0) MAX_SPEED=BLUE_MAX_SPEED;
	else MAX_SPEED=RED_MAX_SPEED;
    static float x, y, w, factor;
    static double max_val;
    if (chassis.enable_switch == false) return; //如果底盘不被使能，则没有后续操作
    short fn;
	static short time_count = 0;
